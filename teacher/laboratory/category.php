...
if (!isset($categories[$id])) {
    redirect('/teacher/laboratory/index.php');
}
...
if ($id === 5) {
    ...
    // For Laboratoriya Imkoniyatlari (category 2), sort numbered items properly. Unnumbered items (0) appear first.
...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
        <div class="flex items-center gap-2 mb-1">
            <a href="<?= BASE_URL ?>/teacher/laboratory/index.php" class="text-gray-400 hover:text-gray-600"><i class="fas fa-arrow-left"></i> Orqaga</a>
        ...
        <?php endif; ?>
            <div class="mt-auto pt-4 border-t border-gray-50">
                <?php if (!empty($item['file_path'])): ?>
                    <a href="<?= BASE_URL ?>/teacher/laboratory/view.php?id=<?= $item['id'] ?>" class="block w-full text-center py-2 bg-<?= $color ?>-50 text-<?= $color ?>-600 hover:bg-<?= $color ?>-100 rounded-lg text-xs font-bold transition">Ko'rish</a>
                <?php elseif (!empty($item['url'])): ?>
                    <?php if ($id === 6): ?>
                        <a href="<?= BASE_URL ?>/teacher/laboratory/view_url.php?id=<?= $item['id'] ?>" class="block w-full text-center py-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg text-xs font-bold transition"><i class="fas fa-play-circle mr-1"></i> Videoni ko'rish</a>
                    <?php else: ?>
                        <a href="<?= BASE_URL ?>/teacher/laboratory/view_url.php?id=<?= $item['id'] ?>" target="_blank" class="block w-full text-center py-2 bg-slate-50 text-slate-600 hover:bg-slate-100 rounded-lg text-xs font-bold transition">Platformaga o'tish</a>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
        </div>
