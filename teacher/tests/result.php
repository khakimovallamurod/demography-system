...
if (!$result || $result['user_id'] != $_SESSION['user_id']) {
    ...
    redirect('/teacher/tests/index.php');
}
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-5">
        ...
        <div class="p-5">
            <div class="flex gap-3">
                <a href="<?= BASE_URL ?>/teacher/tests/take.php?id=<?= $result['test_id'] ?>"
    ...
    <div class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <a href="<?= BASE_URL ?>/teacher/lectures/view.php?id=<?= $test['module_id'] ?>" class="w-full sm:w-auto bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold px-8 py-3.5 rounded-xl transition shadow-sm flex items-center justify-center gap-2">
            <i class="fas fa-arrow-left"></i> Ma'ruzaga qaytish
        </a>
        <?php if ($is_passed): ?>
        <a href="<?= BASE_URL ?>/teacher/lectures/index.php" class="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3.5 rounded-xl transition shadow-md flex items-center justify-center gap-2">
