...
if (is_admin()) redirect("/teacher/dashboard.php");

$id = (int)($_GET['id'] ?? 0);
if ($id <= 0) {
    redirect('/teacher/library/index.php');
}

$book = $db->get_data_by_table('library', ['id' => $id]);
if (!$book || !file_exists(__DIR__ . '/../../' . $book['file_path'])) {
    die_with_swal("Topilmadi", "Kitob topilmadi yoki o'chirilgan.");
}

// Increment views
$db->query("UPDATE library SET views = views + 1 WHERE id = {$id}");

$pdfUrl = BASE_URL . '/' . $book['file_path'];
$page_title = $book['title'] . " - O'qish";
include __DIR__ . '/../../includes/teacher_header.php';
?>

<div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
        <h2 class="text-xl font-bold text-gray-800 line-clamp-1"><?= h($book['title']) ?></h2>
    </div>
    <div class="flex items-center gap-2">
        <a href="<?= BASE_URL ?>/teacher/library/index.php" class="bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 px-4 py-2.5 rounded-xl text-sm font-medium transition shadow-sm flex items-center gap-2">
